...
function exitedSmile() {
  document.getElementById("newGame").innerHTML =
    '<img src="Images/shock.png" id="smiley">';
}

function smile() {
  document.getElementById("newGame").innerHTML =
    '<img src="Images/smile.png" id="smiley">';
}
...
function generateBoard(update) {
  ...
  mines.innerHTML = difficulties[difficulty].mines;
  let tile = {};
  field.style.width = difficulties[difficulty].fieldWidth + "vw";
  field.style.height = difficulties[difficulty].fieldHeight + "vw";
  tile.width =
    difficulties[difficulty].fieldWidth / difficulties[difficulty].width;
  tile.height =
    difficulties[difficulty].fieldHeight / difficulties[difficulty].height;

  field.innerHTML = "";
  for (let i = 0; i < difficulties[difficulty].width; i++) {
    for (let j = 0; j < difficulties[difficulty].height; j++) {
      field.innerHTML +=
        '<button class="unopened tile" style="width: ' +
        tile.width +
        "vw; height: " +
        tile.height +
        "vw; left: " +
...
function fillBoard(difficulty, ele) {
  ...
  for (let i = 0; i < difficulties[difficulty].mines; i++) {
    ...
    while (
      board[row][col] == "mine" ||
      getSurroundingTiles(getTile(ele)).some(
        (e) => e.row == row && e.column == col
      ) ||
      (getTile(ele).row == row && getTile(ele).column == col)
    ) {
...
function uncover(ele) {
  ...
  if (!window.event.ctrlKey) {
    ...
    Array.from(document.getElementsByClassName("tile")).some((e) => {
      if (
        ((e.onclick !== null || tile.value == "mine") &&
          getTile(e).value !== "mine") ||
        (e.classList.contains("flagged") && getTile(e).value !== "mine")
      ) {
        won = false;
        return !won;
      } else {
...
function gameLost(click) {
  ...
  setTimeout(() => {
    document.getElementById("newGame").innerHTML =
      '<img src="Images/lost.png" id="smiley">';
  }, 100);
  board.forEach((row, i) => {
    row.forEach((e, j) => {
      let id = i + "," + j;
...
function gameWon() {
  ...
  setTimeout(() => {
    document.getElementById("newGame").innerHTML =
      '<img src="Images/victory.png" id="smiley">';
  }, 100);

...
color = d3.scaleOrdinal().domain(options.names).range(options.colors);
...
symbolScale = d3.scaleOrdinal().domain(options.names).range(options.categories);
...
function display(data, category) {
  ...
  g.selectAll('rect')
    ...
    .on('mouseover', function (event, element) {
      ...
      d3.select('[id="' + element.date + '"]').attr('r', 8);
...
function init() {
  ...
  d3.json('https://covid.ourworldindata.org/data/owid-covid-data.json').then(
    (data, error) => {
      if (error) {
        console.log(error);
      } else {
